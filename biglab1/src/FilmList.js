import { ListGroupItem, Col,Container, Row, Button, Form } from "react-bootstrap";
...
export function DynamicRating(props){
    ...
    component.push(<BsStar className="pointer" style={{color: "white"}} visibility={"visible"} onClick={() => props.updateStars(props.currentFilm, undefined)} onMouseOver={() => setTemporaryStars(0)} onMouseOut={() => setTemporaryStars(rat)} /*onMouseOut={() => props.setTemporaryStars(props.rating)}*/ key={-1}></BsStar>)
    for(let i = 0; i < rat; i++)
