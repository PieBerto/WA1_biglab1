...
function FilmLibrary(){
    ...
    this.addMovieInDatabase = (movie) =>{
        return new Promise((resolve, reject) => {
            const sqlInsert = 'INSERT INTO FILMS (id, title, favorite, watchdate, rating) VALUES (?, ?, ?, ?, ?)';
            db.run(sqlInsert, [movie.id, movie.title, movie.favorite, movie.watchdate, movie.rating], (err, rows) => {
                if (err){
